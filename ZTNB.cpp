...
double
ZTNBD::expected_saturation(const double initial_distinct,
			   const double initial_sample_size,
			   const double t) {
  const double prob_zero = exp(-log(1.0 + alpha*mu)/alpha);
  const double log_deriv_prob_zero_t = 
    log(mu) - (alpha + 1.0)*log(1.0 + t*alpha*mu)/alpha;

  return exp(log(initial_distinct) - log(initial_sample_size)
	     + log_deriv_prob_zero_t
	     -log(1 - prob_zero));
}
